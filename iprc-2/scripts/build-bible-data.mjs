...
const lexicalPresentationPt = await json('lexical-presentation-pt.json');
const normalizeGloss = value => value?.trim().toLowerCase().replace(/[.,:;!?]+$/g,'') || '';
const glossPtByOriginal = Object.fromEntries(Object.entries(lexicalPresentationPt.gloss).map(([key,value])=>[normalizeGloss(key),value]));
const presentationGloss = (strong,value) => lexicalPresentationPt.strong[strong] || glossPtByOriginal[normalizeGloss(value)] || null;
...
const datasets=[
  ...
  ['greek-tr',groupFlat(greek,'greek-tr',v=>{const imported=greekLexical[`${codes[v.book-1]}/${v.chapter}/${v.verse}`];if(imported){greekLexicalVerses++;const words=imported.map(({id,form,lemma,transliteration,strong,glossOriginal,morphology})=>recordLexicalUsage({id,form,language:'grc',lemma,transliteration,strong,glossOriginal,glossPt:presentationGloss(strong,glossOriginal),morphology,grammar:greekGrammar(morphology),source:'STEPBible',alignment:'verified'}));return{number:v.verse,text:words.map(word=>word.form).join(' '),words,languages:['grc'],languageLabel:'Grego koiné',lexicalAlignment:'verified'}}return{number:v.verse,text:v.text.replace(/ G\d+| [A-Z][A-Z0-9-]+/g,''),words:parseGreek(v.text).map(word=>recordLexicalUsage({...word,glossOriginal:null,glossPt:null,language:'grc',grammar:greekGrammar(word.morphology),source:'TR Parsed',alignment:'unavailable'})),languages:['grc'],languageLabel:'Grego koiné',lexicalAlignment:'unavailable'}})],
  ['hebrew-wlc',groupFlat(hebrew,'hebrew-wlc',v=>{const imported=hebrewLexical[`${codes[v.book-1]}/${v.chapter}/${v.verse}`];if(imported){hebrewLexicalVerses++;const words=imported.map(({id,form,language,lemma,transliteration,strong,lexicalIdentifiers,glossOriginal,morphology})=>{if(language==='arc')aramaicTokens++;return recordLexicalUsage({id,form:form.replaceAll('/',''),language,lemma,transliteration,strong,lexicalIdentifiers:lexicalIdentifiers?.length>1?lexicalIdentifiers:undefined,glossOriginal,glossPt:presentationGloss(strong,glossOriginal),morphology,grammar:semiticGrammar(morphology),source:'OSHB/STEPBible',alignment:'verified'})});const languages=[...new Set(words.map(word=>word.language))];return{number:v.verse,text:words.map(word=>word.form).join(' '),words,languages,languageLabel:languages.includes('arc')?(languages.includes('hbo')?'Hebraico bíblico + Aramaico bíblico':'Aramaico bíblico'):'Hebraico bíblico',lexicalAlignment:'verified'}}return{number:v.verse,text:v.text,words:v.text.split(/\s+/).map(form=>({form,lemma:null,strong:null,morphology:null,transliteration:null,glossOriginal:null,glossPt:null,language:'hbo',source:'WLC',alignment:'unavailable'})),languages:['hbo'],languageLabel:'Hebraico bíblico',lexicalAlignment:'unavailable'}})]
];
