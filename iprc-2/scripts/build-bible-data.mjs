...
const datasets=[
  ...
  ['greek-tr',groupFlat(greek,'greek-tr',v=>{const imported=greekLexical[`${codes[v.book-1]}/${v.chapter}/${v.verse}`];if(imported){greekLexicalVerses++;const words=imported.map(({id,form,lemma,transliteration,strong,gloss,morphology})=>({id,form,language:'grc',lemma,transliteration,strong,gloss,morphology,grammar:greekGrammar(morphology),source:'STEPBible',alignment:'verified'}));return{number:v.verse,text:words.map(word=>word.form).join(' '),words,languages:['grc'],languageLabel:'Grego koiné',lexicalAlignment:'verified'}}return{number:v.verse,text:v.text.replace(/ G\d+| [A-Z][A-Z0-9-]+/g,''),words:parseGreek(v.text).map(word=>({...word,language:'grc',grammar:greekGrammar(word.morphology),source:'TR Parsed',alignment:'unavailable'})),languages:['grc'],languageLabel:'Grego koiné',lexicalAlignment:'unavailable'}})],
  ['hebrew-wlc',groupFlat(hebrew,'hebrew-wlc',v=>{const imported=hebrewLexical[`${codes[v.book-1]}/${v.chapter}/${v.verse}`];if(imported){hebrewLexicalVerses++;const words=imported.map(({id,form,language,lemma,transliteration,strong,lexicalIdentifiers,gloss,morphology})=>{if(language==='arc')aramaicTokens++;return{id,form:form.replaceAll('/',''),language,lemma,transliteration,strong,lexicalIdentifiers:lexicalIdentifiers?.length>1?lexicalIdentifiers:undefined,gloss,morphology,grammar:semiticGrammar(morphology),source:'OSHB/STEPBible',alignment:'verified'}});const languages=[...new Set(words.map(word=>word.language))];return{number:v.verse,text:words.map(word=>word.form).join(' '),words,languages,languageLabel:languages.includes('arc')?(languages.includes('hbo')?'Hebraico bíblico + Aramaico bíblico':'Aramaico bíblico'):'Hebraico bíblico',lexicalAlignment:'verified'}}return{number:v.verse,text:v.text,words:v.text.split(/\s+/).map(form=>({form,lemma:null,strong:null,morphology:null,transliteration:null,language:'hbo',source:'WLC',alignment:'unavailable'})),languages:['hbo'],languageLabel:'Hebraico bíblico',lexicalAlignment:'unavailable'}})]
];
...
const manifest={generatedAt:report.generatedAt,default:{version:'blivre',book:'JHN',chapter:11},books,versions:[
 ...
 {id:'greek-tr',name:'Texto original',abbreviation:'Texto original',language:'grc',languageLabel:'Grego koiné',direction:'ltr',testaments:['NT'],source:'Textus Receptus + STEPBible TAGNT/TBESG',sourceDate:'STEPBible commit 079db44 (2026-08-13)',license:'TR em domínio público; dados STEPBible CC BY 4.0',attribution:'Textus Receptus Parsed NT; STEP Bible — www.STEPBible.org',status:'verified',enabled:true,features:['parallel','strongs','lemma','transliteration','gloss','morphology','token-language']},
 {id:'hebrew-wlc',name:'Texto original',abbreviation:'Texto original',language:'hbo-arc',languageLabel:'Hebraico bíblico / Aramaico bíblico',direction:'rtl',testaments:['AT'],source:'WLC + Open Scriptures Hebrew Bible + STEPBible TBESH',sourceDate:'OSHB commit 3d15126; STEPBible commit 079db44 (2026-08-13)',license:'WLC em domínio público; OSHB lemma/morfologia e STEPBible CC BY 4.0',attribution:'Open Scriptures Hebrew Bible Project; STEP Bible — www.STEPBible.org',status:'verified',enabled:true,features:['parallel','strongs','lemma','transliteration','gloss','morphology','token-language']},
