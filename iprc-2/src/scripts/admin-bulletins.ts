...
const addItem = (collection: string, item: Json = {}) => {
  ...
  if (collection === 'birthdays') add(row, field('wide', 'Nome', 'text', text(item.name)), field('', 'Data', 'date', text(item.date)));
...
function editorValue(): Json {
  ...
  const birthdays = rows('birthdays').map((row, sortOrder) => { const [name, date] = inputs(row); return { id: row.dataset.id, name, date, source: 'manual', sortOrder }; });
...
async function save(manual = false) {
  ...
  try { const input = editorValue(); const saved = current?.id ? await api(`/api/admin/bulletins/${encodeURIComponent(current.id)}`, { method: 'PUT', body: JSON.stringify({ value: input, expectedUpdatedAt: current.updatedAt }) }) : await api('/api/admin/bulletins', { method: 'POST', body: JSON.stringify(input) }); current = saved; fillEditor(saved); await refreshList(); saveStatus.textContent = manual ? 'Salvo agora' : 'Alterações salvas automaticamente'; }
...
function renderPreview() {
  ...
  for (const [label, entries, describe] of [['Avisos', item.announcements, (entry: Json) => `${entry.title}: ${documentText(entry.content)}`], ['Atividades', item.monthActivities, (entry: Json) => entry.text], ['Aniversariantes', item.birthdays, (entry: Json) => `${entry.name} — ${formatDate(entry.date)}`], ['Escala diaconal', item.diaconalSchedule, (entry: Json) => `${formatDate(entry.date)} — ${entry.responsible.join(', ')}`], ['Leituras', item.weeklyReadings, (entry: Json) => `${entry.day}: ${entry.referenceText}`]] as any[]) { if (!entries.length) continue; const section = document.createElement('section'); const h = document.createElement('h3'); h.textContent = label; const ul = document.createElement('ul'); entries.forEach((entry: Json) => { const li = document.createElement('li'); li.textContent = describe(entry); add(ul, li); }); add(section, h, ul); add(digital, section); }
  q('[data-print-cover]').textContent = `Boletim ${item.number}\n${formatDate(item.date)}`; q('[data-print-pastoral]').textContent = `${item.pastoral.title}\n${documentText(item.pastoral.body)}`; q('[data-print-announcements]').textContent = item.announcements.map((entry: Json) => `${entry.title}: ${documentText(entry.content)}`).join('\n'); q('[data-print-info]').textContent = [...item.monthActivities.map((entry: Json) => entry.text), ...item.weeklyReadings.map((entry: Json) => `${entry.day}: ${entry.referenceText}`)].join('\n');
