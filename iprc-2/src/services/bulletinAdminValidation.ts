...
export const normalizeBulletinAdminInput=(raw:unknown):BulletinInput=>{
  ...
  value.birthdays=(value.birthdays||[]).map((item,index)=>({...item,id:text(item.id,100),name:text(item.name,180),date:text(item.date,10),source:'manual',memberId:undefined,visibility:['hidden','print','public'].includes(item.visibility)?item.visibility:'print',sortOrder:index}));
