...
if(root){
  ...
  const referenceLabel=ref=>{
    const start=bookByCode(ref.book),end=bookByCode(ref.endBook);
    if(!start||!end)return ref.source;
    if(ref.book===ref.endBook&&ref.chapter===ref.endChapter)return `${start.name} ${ref.chapter}:${ref.verse}${ref.endVerse!==ref.verse?`–${ref.endVerse}`:''}`;
    if(ref.book===ref.endBook)return `${start.name} ${ref.chapter}:${ref.verse}–${ref.endChapter}:${ref.endVerse}`;
    return `${start.name} ${ref.chapter}:${ref.verse}–${end.name} ${ref.endChapter}:${ref.endVerse}`;
  };
  const initial=()=>{
