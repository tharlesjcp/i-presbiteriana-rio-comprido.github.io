...
Novos estudos são criados em `/admin/estudos` com dados editoriais humanos e começam sempre em `draft`. `slug` e `youtubeId` são derivados; `summary`, conteúdo editorial, transcript e referências são opcionais. Publicar, despublicar, arquivar e restaurar são ações explícitas. Os oito registros iniciais da migration `0007_studies.sql` correspondem somente aos vídeos reais confirmados nesta fase.
