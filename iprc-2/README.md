...
O contrato de domínio está em `src/domain/study.ts`. `StudyInput` representa a entrada editorial humana: título, data, URL comum do YouTube, referências, status e, opcionalmente, ID, slug, resumo, conteúdo editorial e transcrição. `Study` é a entidade normalizada, com `slug` definitivo e `youtubeId` derivado automaticamente. A normalização valida os campos, descarta resumo vazio, gera ou valida o slug e extrai o ID do vídeo sem exigir que o editor o descubra. A versão editorial usa um subconjunto seguro de Markdown (`##` e parágrafos), renderizado sem HTML arbitrário.

As páginas dependem da interface `src/repositories/StudyRepository.ts`, não da persistência. `D1StudyRepository` fornece o acervo público por meio do Worker e `D1StudyAdminRepository` atende o painel protegido por Access, preservando publicação explícita, arquivo e concorrência otimista. `StaticStudyRepository` permanece como implementação local sem conteúdo público fictício.

Novos estudos são criados em `/admin/estudos` com dados editoriais humanos e começam sempre em `draft`. `slug` e `youtubeId` são derivados; `summary`, conteúdo editorial, transcript e referências são opcionais. A edição de um estudo publicado altera somente a entidade administrativa: o site continua lendo o snapshot ativo em `study_publications` até **Republicar**. Despublicar e arquivar retiram a revisão pública sem apagar o conteúdo nem o histórico. Os oito registros iniciais da migration `0007_studies.sql` correspondem somente aos vídeos reais confirmados nesta fase. O fluxo manual e sua rastreabilidade estão documentados em `docs/study-content-audit.md`.

O preview de referências carrega sob demanda os artefatos BLIVRE já gerados por capítulo em `public/bible-data`, sem duplicar textos. O mesmo diálogo acessível é reutilizado por Estudos, referência principal da Pastoral e Leituras da semana dos Boletins.
...
`BulletinRepository` isola as páginas da persistência. Em Cloudflare, `D1BulletinRepository` expõe somente o snapshot ativo de `bulletin_publications`; o autosave altera a entidade editorial sem modificar a edição pública. Publicar, republicar e despublicar são comandos explícitos, com número único, slug estável, snapshots imutáveis e auditoria. O fluxo completo está em `docs/bulletin-editorial-flow.md`.
